...
export default function DownloadGuide() {
  ...
  return (
    <section id="downloads" className="py-16 md:py-24 bg-gray-50 dark:bg-zinc-900 transition-colors duration-200 selection:bg-cyan-500/20">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="max-w-3xl mx-auto text-center mb-12">
          ...
          <p className="text-sm text-gray-500 dark:text-zinc-400 mt-4 leading-relaxed">
            支持原生跨平台运作速度，提供极简的 GUI 双击安装包以及符合 DevOps 习惯的命令行包管理器服务。
          </p>
          ...
          {detectedOS && (
            <div className="mt-5.5 inline-flex items-center space-x-2 bg-gradient-to-r from-cyan-500/10 to-blue-500/10 border border-cyan-500/20 px-4 py-1.5 text-xs text-cyan-700 dark:text-cyan-400 font-medium">
              <Laptop size={13.5} />
              <span>系统识别：智能为您推荐 <strong>{detectedOS === 'mac' ? 'macOS' : detectedOS === 'windows' ? 'Windows' : 'Linux'}</strong> 安装包</span>
            </div>
          )}
        </div>
            ...
            return (
              ...
              >
                ...
                <div>
                  <div className="flex items-start justify-between mb-4">
                    <div>
                      ...
                      <p className="text-[10.5px] text-gray-400 mt-1 font-mono">{pt.architecture}</p>
                    </div>
                  ...
                  <div className="grid grid-cols-2 gap-4 py-3 pb-4 border-y border-gray-100 dark:border-zinc-850 text-xs mb-5">
                    <div>
                      <span className="text-gray-400 block text-[10px] uppercase font-bold tracking-wide">存储积体</span>
                      <span className="font-semibold text-gray-700 dark:text-zinc-300 font-mono">{pt.fileSize}</span>
                    </div>
                    <div>
                      <span className="text-gray-400 block text-[10px] uppercase font-bold tracking-wide">包形式</span>
                      <span className="font-semibold text-gray-700 dark:text-zinc-300 font-mono">{pt.fileFormat}</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-3.5">
                  ...
                  <button
                    onClick={() => alert(`您正在模拟下载【${pt.name}】安装包...\n这会激活标准二进制包分发流程，包含 SHA256 加密验证。`)}
        ...
        <div className="max-w-xl mx-auto mt-12 text-center text-[11px] text-gray-400 bg-white dark:bg-zinc-950 border border-gray-150 dark:border-zinc-850 px-4 py-3">
          💡 <strong>安全性声明：</strong> WinSSH 官方承诺所有架构的分发文件均在本地沙箱进行安全验证，没有任何外部服务器上传行迹，您可以安全放心地连接生产数据库和跳板堡垒节点。
        </div>
