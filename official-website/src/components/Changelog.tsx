...
function ReleaseItem({ release, index, formatDate, t }: { release: Release; index: number; formatDate: (d: string) => string; t: any }) {
    ...
    >
      ...
      <div className="w-[calc(100%-2rem)] md:w-[calc(50%-2rem)] p-6 rounded-2xl bg-white/5 border border-white/10 hover:bg-white/10 transition-colors">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-4 gap-2">
          <div className="flex items-center gap-3 flex-wrap">
            <h3 className="font-display font-semibold text-xl text-white">
              <a href={release.html_url} target="_blank" rel="noopener noreferrer" className="hover:text-cyan-400 transition-colors">
          ...
          <time className="text-sm font-mono text-neutral-500">{formatDate(release.published_at)}</time>
        </div>
        
        <div 
          className={`mt-4 break-words relative transition-all duration-300 ${!expanded && isLong ? 'max-h-[200px] overflow-hidden' : ''}`}
          style={!expanded && isLong ? { WebkitMaskImage: 'linear-gradient(to bottom, black 50%, transparent 100%)', maskImage: 'linear-gradient(to bottom, black 50%, transparent 100%)' } : {}}
        >
          <ReactMarkdown
            components={{
              ...
              p: ({node, ...props}) => <p className="text-neutral-400 text-sm font-light mb-3 last:mb-0" {...props} />,
              h1: ({node, ...props}) => <h4 className="text-white font-medium text-base mb-2 mt-6 first:mt-0" {...props} />,
              h2: ({node, ...props}) => <h4 className="text-white font-medium text-base mb-2 mt-6 first:mt-0" {...props} />,
              h3: ({node, ...props}) => <h4 className="text-white font-medium text-sm text-neutral-300 mb-2 mt-6 first:mt-0" {...props} />,
              a: ({node, ...props}) => <a className="text-cyan-400 hover:underline" target="_blank" rel="noopener noreferrer" {...props} />,
              code: ({node, ...props}) => <code className="bg-white/10 px-1 py-0.5 rounded text-xs font-mono text-neutral-300" {...props} />,
        ...
        {isLong && (
          ...
          >
             {expanded ? <><ChevronUp className="w-4 h-4"/> {t.changelogSection.showLess || 'Show less'}</> : <><ChevronDown className="w-4 h-4"/> {t.changelogSection.showMore || 'Read more'}</>}
          </button>
        )}
      </div>
    </motion.div>
    ...
    <section id="changelog" className="py-32 px-6 relative z-10 border-t border-white/5">
      <div className="max-w-4xl mx-auto relative z-10">
        <div className="mb-20">
          ...
          >
            {t.changelogSection.titleLevel1} <span className="text-transparent bg-clip-text bg-gradient-to-r from-neutral-300 to-white">{t.changelogSection.titleLevel2}</span>
          </motion.h2>
          <motion.p
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
        ...
        <div className="space-y-16 relative before:absolute before:inset-0 before:ml-2.5 before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-gradient-to-b before:from-transparent before:via-white/10 before:to-transparent">
          ...
          ) : releases.slice(0, visibleCount).map((release, i) => (
            <ReleaseItem 
              key={release.id} 
              release={release} 
              index={i} 
