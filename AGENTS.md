...
### 3. SFTP 面板已经从单文件浏览器升级成更完整的远端资源面板
...
密码和 passphrase 仍然经由 `src/main/secure-store.ts` 走系统安全存储，不直接落 SQLite。
...
其中 `Pixel CRT` 不只是配色，还附带终端默认字体策略。
...
1. 做真实 SSH / SFTP / 端口转发联调，确认新连接 phase、私钥内容存储和远程文件操作边界
2. 决定私钥内容入库是否是长期方案，还是后续需要加密存储或回到安全文件引用模型
3. 决定端口转发规则是否需要持久化、模板化或跨 session 复用
4. 继续完善 SFTP 资源管理，尤其是非空目录、批量操作和错误反馈
5. 持续打磨主题系统与用户主题安装流程
