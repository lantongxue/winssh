...
WinSSH 是一个基于 Electron、React 和 TypeScript 的跨平台桌面 SSH 工作台。它把保存的服务器、SSH 会话、本地终端、SFTP、端口转发、主题系统、WebDAV 备份恢复和应用更新放进同一个桌面应用里；仓库同时包含 `web/` 品牌站与文档入口页。
...
- 核心运行时：xterm.js、ssh2、node-pty、better-sqlite3、electron-updater、keytar
...
- SFTP：目录浏览、上传、下载、新建文件、重命名、删除、拖拽上传、多选
...
- 根据仓库内 `AGENTS.md` 的 `2026-04-25` 快照，`npm run typecheck`、指定 Vitest 测试和 `npm run build` 已通过
