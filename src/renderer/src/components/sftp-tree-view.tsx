...
function SftpTreeEntryRow({
  ...
  const { onClick, onDoubleClick } = useSingleOrDoubleClick({
    onSingleClick: () => onSelectSingleEntry(entry.path),
    onDoubleClick: () => {
      if (isDirectory) {
      ...
      <ContextMenu>
        ...
        <ContextMenuContent>
          {singleContextTarget?.kind === 'directory' ? (
            <ContextMenuItem onClick={() => onToggleExpanded(singleContextTarget.path)}>
          ...
          {singleContextTarget ? (
            <ContextMenuItem
              onClick={() => {
                onSetRenameTarget(singleContextTarget)
              }}
            >
...
export interface SftpTreeViewHandle {
  ...
  getNodeCount: () => number
  selectEntryRange: (path: string, anchorPath: string | null, lastSelectedPath: string | null) => {
    paths: string[]
    anchorPath: string
  }
}
...
export const SftpTreeView = forwardRef<SftpTreeViewHandle, SftpTreeViewProps>(
  function SftpTreeView(
    {
      session,
      entries,
      viewMode,
